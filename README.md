...
Following Docker images (tags) are created after successful Travis CI build:
 ...
 - on any commit - `travis-[build #]` and commit hash i.e.: `afd348f`
 - on _master_ commit - with tag `latest`
 - on other branches - with `wip-` prefix (Work In Progress) and followed by the branch name, i.e.: `wip-development`
 - on git tag creation - additionally to above tags will be added:
